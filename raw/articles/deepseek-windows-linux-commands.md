---
source_url: https://chat.deepseek.com/share/9itr8yzwo2jwvq4naf
ingested: 2026-06-21
sha256: e790c3b91eb265761c1074bb17835256f9198cd80994c1ca0f36d75e17bf0520
---

# DeepSeek 共享对话：常见 Windows 命令和 Linux 命令

> 来源：DeepSeek 共享对话 https://chat.deepseek.com/share/9itr8yzwo2jwvq4naf

## 用户提问
常见的 Windows 命令和 Linux 命令

## AI 思考过程
用户需要一份 Windows 和 Linux 的常见命令清单。考虑到命令数量多，按功能分类整理会更清晰。可以同时搜索两份清单，这样能快速获取基础列表。

搜索结果里关于 Windows 和 Linux 命令的页面不少，看起来都是分类整理的列表。为了获取具体命令和分类细节，需要打开这些页面看看。

这些资料提供了丰富的命令列表和分类。回答按功能分类整理 Windows 和 Linux 的常见命令，并提供一个对比表格。
...
| md / mkdir | 创建新目录 |
| rd / rmdir | 删除目录（默认只能删空目录，/s 可删除目录及所有内容） |
| copy | 复制文件 |
...
小提示：在 Windows 中查看命令帮助可用 `命令 /?`；在 Linux 中可用 `man 命令` 或 `命令 --help`。
